...
def isolated(monkeypatch, tmp_path):
    ...
    requests=[]; mutate=[None]; proxy=[False]
    def open_local(request,timeout):
        ...
        if proxy[0]:
            headers={key:value for key,value in headers.items() if key.lower()!='x-request-id'}
            headers['X-Request-Id']=uuid.uuid4().hex
        response=client.request(request.get_method(),path,headers=headers,content=request.data)
        data=response.json()
        ...
        if response.status_code in (401,403,404):
            if mutate[0]=='mismatch':data['request_id']='different-safe-server-id'
            elif mutate[0]=='missing':del response.headers['X-Request-Id']
            elif mutate[0]=='invalid':
                response.headers['X-Request-Id']='bad id'
                data['request_id']='bad id'
    ...
    try:
        yield SimpleNamespace(execute=execute,requests=requests,manifest=manifest,mutate=mutate,proxy=proxy,db=db,output=tmp_path/'evidence.json')
    finally:
...
def test_proxy_overwrites_client_id_preserving_server_correlation(isolated):
    isolated.proxy[0]=True
    evidence=isolated.execute()
    assert evidence['status']=='passed'
    rows=[row for row in evidence['requests'] if row['path'].startswith('/api/review/tasks/')]
    assert len(rows)==30 and all(row['request_id']!=row['response_request_id'] for row in rows)
    assert all(row['status']=='passed' for row in rows)


@pytest.mark.parametrize('case',['mismatch','missing','invalid'])
def test_server_response_id_must_be_valid_and_match_body(isolated,case):
    isolated.proxy[0]=True
    isolated.mutate[0]=case
    with pytest.raises(RuntimeError):isolated.execute()
    row=json.loads(isolated.output.read_text())['requests'][-1]
    assert row['actual']==401 and row['status']=='failed'
