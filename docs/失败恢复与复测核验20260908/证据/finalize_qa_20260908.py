...
MARKER = "20260908"
ROLE_CODE = "qa_permission_20260908"
ROLE_ID = 7
PROJECTS = {164: 107, 165: 109}
FILES = {1894: 164, 1895: 165}
PERMISSIONS = {
...
CREDENTIALS = Path("/root/prism-acceptance-20260908/credentials.json")
RESULT = CREDENTIALS.parent / "finalize-qa-result.json"
...
def validate_manifest(manifest):
    ...
    require(type(manifest.get("role_id")) is int and manifest["role_id"] == ROLE_ID, "invalid_role_id")
...
def snapshot(role_id):
    ...
    require(type(role_id) is int and role_id == ROLE_ID, "invalid_role_id")
    sql = f"""
SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ;
SET SESSION TRANSACTION READ ONLY;
START TRANSACTION WITH CONSISTENT SNAPSHOT, READ ONLY;
SELECT 'context', JSON_OBJECT('database',DATABASE(),'readonly',@@session.transaction_read_only);
SELECT 'users', COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id',id,'username',username,'role',role,'status',status,'token_version',token_version)),JSON_ARRAY()) FROM `user` WHERE id IN (107,108,109,110);
SELECT 'role', COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id',id,'code',code,'status',status,'is_builtin',is_builtin)),JSON_ARRAY()) FROM `role` WHERE id={role_id} OR code='qa_permission_20260908';
SELECT 'assignments', COALESCE(JSON_ARRAYAGG(JSON_OBJECT('user_id',ur.user_id,'role_id',ur.role_id,'code',r.code)),JSON_ARRAY()) FROM user_role ur LEFT JOIN `role` r ON r.id=ur.role_id WHERE ur.user_id IN (107,108,109,110) OR ur.role_id={role_id};
SELECT 'permissions', COALESCE(JSON_ARRAYAGG(p.code),JSON_ARRAY()) FROM role_permission rp LEFT JOIN permission p ON p.id=rp.permission_id WHERE rp.role_id={role_id};
SELECT 'projects', COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id',id,'user_id',user_id,'marker_match',project_name=CASE id WHEN 164 THEN 'QA权限验收-20260908-a' WHEN 165 THEN 'QA权限验收-20260908-b' END,'status',status,'row_sha256',SHA2(CAST(JSON_ARRAY(id,user_id,project_name,description,language,status,create_time,update_time) AS CHAR CHARACTER SET utf8mb4),256))),JSON_ARRAY()) FROM project WHERE id IN (164,165) OR user_id IN (107,108,109,110);
SELECT 'memberships', COALESCE(JSON_ARRAYAGG(JSON_OBJECT('project_id',project_id,'user_id',user_id,'role_in_project',role_in_project)),JSON_ARRAY()) FROM project_member WHERE project_id IN (164,165) OR user_id IN (107,108,109,110);
SELECT 'files', COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id',id,'project_id',project_id,'status',status,'row_sha256',SHA2(CAST(JSON_ARRAY(id,project_id,file_name,file_path,language,size_bytes,line_count,version_no,status,is_binary,raw_size,create_time,update_time,SHA2(content,256),SHA2(original_blob,256)) AS CHAR CHARACTER SET utf8mb4),256))),JSON_ARRAY()) FROM code_file WHERE id IN (1894,1895) OR project_id IN (164,165);
SELECT 'counts', JSON_OBJECT('ai_call_log',(SELECT COUNT(*) FROM ai_call_log WHERE user_id IN (107,108,109,110)),'review_task',(SELECT COUNT(*) FROM review_task WHERE user_id IN (107,108,109,110) OR project_id IN (164,165)));
ROLLBACK;
"""
    ...
    require(set(parsed) == {"context", "users", "role", "assignments", "permissions", "projects", "memberships", "files", "counts"}, "incomplete_snapshot")
    require(parsed["context"] == {"database": "code_review", "readonly": 1}, "snapshot_not_read_only")
    for key in ("users", "role", "projects", "files"):
        parsed[key].sort(key=lambda row: row["id"])
    parsed["assignments"].sort(key=lambda row: (row["user_id"], row["role_id"]))
    parsed["memberships"].sort(key=lambda row: (row["project_id"], row["user_id"]))
    parsed["permissions"].sort()
    return parsed


def validate_snapshot(value, manifest, *, status):
    require(value["context"] == {"database": "code_review", "readonly": 1}, "snapshot_not_read_only")
    ...
    require(value["permissions"] == sorted(PERMISSIONS), "qa_permissions_changed")
    require(len(value["projects"]) == 2, "qa_project_missing")
    for row, project_id, owner_id in zip(value["projects"], (164, 165), (107, 109)):
        ...
        require(valid_hash(row["row_sha256"]), "invalid_project_hash")
    require(value["memberships"] == [
        {"project_id": 164, "user_id": 107, "role_in_project": "owner"},
        {"project_id": 164, "user_id": 108, "role_in_project": "reviewer"},
        {"project_id": 165, "user_id": 109, "role_in_project": "owner"},
    ], "qa_project_memberships_changed")
    require(len(value["files"]) == len(FILES), "qa_file_count_changed")
    for row, (file_id, project_id) in zip(value["files"], FILES.items()):
        require(row["id"] == file_id and row["project_id"] == project_id and row["status"] == "active", "qa_file_identity_changed")
        require(valid_hash(row["row_sha256"]), "invalid_file_hash")
...
def http(client, method, path, *, payload=None, token=None):
    ...
    headers = {"Accept": "application/json", "X-Request-Id": "qa-finalize-20260908"}
...
try:
    ...
    done=subprocess.run([sys.executable,str(script),'--marker','20260908','--credentials',str(path),'--disable'],capture_output=True,timeout=60)
    if done.returncode!=0: raise RuntimeError()
    result=json.loads(done.stdout)
    if result!={'status':'disabled','account_ids':[107,108,109,110]}: raise RuntimeError()
...
def finalize(manifest, client, report, save):
    ...
    report["snapshots"]["before_disable"] = before
    require(before["projects"] == initial["projects"], "project_changed_during_login")
    require(before["files"] == initial["files"], "file_changed_during_login")
    ...
    report["snapshots"]["after_disable"] = after
    require(after["projects"] == before["projects"], "project_changed_during_disable")
    require(after["files"] == before["files"], "file_changed_during_disable")
    ...
    for name, entry in zip(ACCOUNTS, report["accounts"]):
        ...
        save()
    tokens.clear()
    report.update(status="passed", stage="finished", model_and_review_counts=after["counts"], retained_projects=after["projects"], retained_files=after["files"], retained_memberships=after["memberships"], retained_permission_codes=after["permissions"], credentials_logged=False, model_requests=0, disabled_account_ids=list(ACCOUNTS.values()))
    save()
...
def main(argv=None):
    ...
    if not args.execute:
        print(json.dumps({"status": "prepared_not_executed", "marker": MARKER, "role_id": ROLE_ID, "account_ids": list(ACCOUNTS.values()), "base_url": BASE, "old_token_expected_http": 403, "old_token_expected_business_code": 40301, "retained_project_ids": list(PROJECTS), "retained_file_ids": list(FILES), "credentials_read": False, "production_requests": 0, "prepare_script_sha256": PREPARE_SHA256}))
        return 0
