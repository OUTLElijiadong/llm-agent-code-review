...
当前候选已完成本地实现与自动化门禁；生产发布、数据治理、真实模型和浏览器证据将在不可变提交发布后回填。本文件不以单元测试替代生产实跑。
...
| DeepSeek 截断 | 画像输出预算传入真实调用；失败分片与失败文件继续后续项；最终失败且保留部分发现 | 通过自动化，真实付费运行待生产核验 |
| 规则快照 | 新任务冻结完整内容和 SHA-256；合法空快照不回查实时规则；旧不完整任务兼容回查 | 通过 |
| 输入边界 | Schema 长度、治理枚举 fail-closed、模板沙箱、上传前置容量、目录逐文件流式上限 | 通过 |
| 角色范围 | 可分配角色仅用户、评审员、管理员；最高管理员唯一；评审员保留 Agent 自定义与渗透授权 | 通过代码与迁移测试，生产待重算 |
...
- QA 临时账号软停用与前后重算：待回填。
- 公网页面截图及知识库标注：待回填。
- 真实 DeepSeek 项目级运行：待回填；若额度或模型配置拒绝，必须保留为明确缺口。
