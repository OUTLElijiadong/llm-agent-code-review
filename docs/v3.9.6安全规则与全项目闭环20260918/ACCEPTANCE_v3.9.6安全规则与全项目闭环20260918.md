...
| 输入边界 | Schema 长度、治理枚举 fail-closed、模板沙箱、上传前置容量、普通 JSON/API 4MiB 网关上限、目录逐文件流式上限 | 通过；新增边界回归 49/49；生产 5MiB JSON 返回结构化 `413 application/json` |
...
- 后端最终全量：本轮输入边界与 JSON 限流补丁完成后，收集 4194 项，4189 项通过、5 项跳过、0 项失败（114.28 秒）。
