...
| AC-O03 | 同账号跨 surface 活动会话硬上限 10 | 通过 | 用户行 `FOR UPDATE`；优先归档最旧空闲；全占用时第 11 条 409 |
...
- 生产状态：`v3.9.10 / 8d0efdcffb2a...` 已通过备份恢复、Alembic、health/ready 与 HTTPS 门禁；发布后并行的一次性 Alembic 容器触发 `ops-check` 误报，已修复为只识别常驻服务容器，目标补丁版本为 `v3.9.11`。
