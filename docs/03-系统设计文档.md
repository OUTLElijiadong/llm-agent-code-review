...
│   ORM        │  │ AgentRegistry (12个)  │  │ ReportLab    │
│ (10张表)     │  │ EventBus + Clarify   │  │              │
└──────────────┘  └──────────────────────┘  └──────────────┘
        │                       │
        ▼                       ▼
┌──────────────┐         ┌──────────────────┐
...
| Agent 中心 | `AgentService`, `Orchestrator`, `AgentRegistry`, `EventBus`, `ClarifyStore` | 12个Agent注册、运行时态势、SSE事件推送、主动追问回填 (v2.0) |
...
| LLM | DeepSeek Chat | 中文代码理解强、价格友好、上下文 32K |
...
| 风险 | 防御措施 |
| --- | --- |
| 上传可执行文件 | 扩展名白名单 + MIME 检测 |
| 上传超大文件 | 后端 `MAX_UPLOAD_SIZE` 限制 200KB,前端预检 |
