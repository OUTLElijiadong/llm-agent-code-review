...
ROLLBACK;
"""
    raw = run(["docker", "exec", "-i", "cr_mysql", "sh", "-c", 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" exec mysql --protocol=TCP -h127.0.0.1 -uroot --database=code_review --batch --raw --skip-column-names'], input=sql.encode())
    parsed = {}
