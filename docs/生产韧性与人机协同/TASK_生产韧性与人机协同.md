...
| T6 | 调度恢复与秒级心跳 | 默认任务表达式、运行记录 | 秒级 interval 注册、孤儿运行回收、部分失败可继续 | T1、T5 | 失败复现用例、自然调度运行记录 |
| T7 | FlyTrap 外部集成降级 | agent/sync 服务日志、现有安全巡检 | 合法事件过滤、健康摘要、管理员告警与人工恢复动作 | T5、T6 | 失败先行测试、真实生产巡检与告警记录 |
