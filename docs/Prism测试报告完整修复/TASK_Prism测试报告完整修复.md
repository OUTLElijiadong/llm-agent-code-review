...
- 生产验收不触发付费模型自动运维，不修改真实用户密码，不删除真实业务数据；临时验收账号在完成角色验证后软删除。

## 本轮执行状态

- [x] T1-T8：缺陷复现、实现和定向回归完成。
- [x] T9：后端 `2039` 项、前端 `370` 项、Lint/Ruff/类型检查/构建完成。
- [x] T12-T13：权限可见性、未知路由、虚拟点击和视觉交互实现及自动化回归完成。
- [x] T10：以 `eccab68a24c9e280d6d4bc3111f800f6f2e69ecd` 执行生产备份、迁移和前后端服务切换；备份恢复验证、85 张表、迁移 `043_remote_import_lifecycle` 和健康检查均通过。
- [x] T11：管理员、审查员、普通用户关键路径真实点击、权限失败关闭、小菱新对话与零模型成本本地导航、控制台检查通过。子 Agent/圆桌/远程导入使用本轮全量与定向自动化验证，未重复触发付费线上长链路。
