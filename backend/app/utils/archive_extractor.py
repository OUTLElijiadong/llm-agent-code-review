...
from app.utils.file_validator import BLOCK_DIRS

# ============ 解压限制配置 ============
MAX_EXTRACTED_FILES = 100        # 解压后文件数量上限
MAX_TOTAL_SIZE = 50 * 1024 * 1024  # 50MB 解压后总大小上限
MAX_SINGLE_FILE_SIZE = 10 * 1024 * 1024  # 10MB 单文件大小上限
...
def _validate_path(name: str) -> Optional[str]:
    ...
    # 过滤敏感目录
    if BLOCK_DIRS.search(normalized):
        return None
