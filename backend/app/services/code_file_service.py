...
def _upload_archive(
    ...
    extracted_total = sum(file.size for file in extracted_files)
    current_total = _get_project_total_size(db, project_id)
    if not validate_project_total_size(current_total, extracted_total):
        raise ValueError(
            f"压缩包解压后项目总大小 {current_total + extracted_total} 字节超过项目上限 "
            f"{MAX_PROJECT_TOTAL_SIZE} 字节(500MB)"
        )
