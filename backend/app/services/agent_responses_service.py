...
def _instructions(surface: str) -> str:
    identity = "Prism 管理员 Agent" if surface == "admin" else "Prism 代码审查 Agent「棱镜小助」"
    ...
    batch_instruction = (
        ...
        if surface == "admin"
        else "你服务的对象主要是不会看文档的普通用户和审查员：回答要像带路人，"
        "先给结论,再给傻瓜式下一步,并把对应页面入口用站内链接标出来；"
        "审查员发起审查、处理问题、导出报告时,优先直接用工具替他完成,再引导到结果页面核对。"
    )
    guide_block = admin_guide_block() if surface == "admin" else user_guide_block()
    return (
