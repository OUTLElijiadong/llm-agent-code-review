...
USER_CAPABILITIES: tuple[UserCapabilitySpec, ...] = (
    ...
    _cap("projects.delete", "/projects", "删除项目", "DELETE", "/api/projects/{project_id}", CRITICAL, "project:delete"),
    _cap("projects.import_remote", "/projects", "导入公开 HTTPS 源码归档", "POST", "/api/projects/import-remote", WRITE, "project:import"),
    ...
    _cap("project_members.update_role", "/projects", "更新项目成员角色", "PUT", "/api/projects/{project_id}/members/{user_id}", WRITE),
    _cap("project_members.remove", "/projects", "移除项目成员", "DELETE", "/api/projects/{project_id}/members/{user_id}", CRITICAL),
    ...
    _cap("code_files.meta", "/code", "查询代码文件元数据", "GET", "/api/code-files/{file_id}/meta", permission="file:view"),
    _cap("code_files.update", "/code", "更新代码文件并创建新版本", "PUT", "/api/code-files/{file_id}", WRITE, "file:edit"),
    _cap("code_files.rename", "/code", "重命名代码文件", "POST", "/api/code-files/{file_id}/rename", WRITE, "file:edit"),
    _cap("code_files.delete", "/code", "删除代码文件", "DELETE", "/api/code-files/{file_id}", CRITICAL, "file:delete"),
    _cap("code_files.versions.list", "/code", "查询代码文件版本", "GET", "/api/code-files/{file_id}/versions", permission="file:view"),
    _cap("code_files.versions.get", "/code", "查询指定代码文件版本", "GET", "/api/code-files/{file_id}/versions/{version_no}", permission="file:view"),
    _cap("code_files.versions.restore", "/code", "回滚代码文件到指定版本", "POST", "/api/code-files/{file_id}/versions/{version_no}/restore", CRITICAL, "file:edit"),
    # 审查任务与问题
    ...
    _cap("reviews.issues", "/reviews", "查询审查任务问题", "GET", "/api/review/tasks/{task_id}/issues", permission="issue:view"),
    _cap("reviews.start", "/reviews", "启动代码审查", "POST", "/api/review/start", WRITE, "review:start"),
    _cap("reviews.cancel", "/reviews", "取消运行中的审查任务", "POST", "/api/review/tasks/{task_id}/cancel", WRITE, "review:cancel"),
    _cap("reviews.delete", "/reviews", "删除审查任务", "DELETE", "/api/review/tasks/{task_id}", CRITICAL, "review:cancel"),
    ...
    _cap("issues.update_status", "/issues", "更新问题状态", "PUT", "/api/issues/{issue_id}/status", WRITE, "issue:handle"),
    _cap("issues.batch_update_status", "/issues", "批量更新问题状态", "POST", "/api/issues/batch-status", WRITE, "issue:batch"),
    ...
    _cap("report_templates.list", "/report/templates", "查询报告模板", "GET", "/api/reports/templates", permission="report:template_manage"),
    _cap("report_templates.create", "/report/templates", "创建报告模板", "POST", "/api/reports/templates", WRITE, "report:template_manage"),
    _cap("report_templates.update", "/report/templates", "更新报告模板", "PUT", "/api/reports/templates/{template_id}", WRITE, "report:template_manage"),
    _cap("report_templates.delete", "/report/templates", "删除报告模板", "DELETE", "/api/reports/templates/{template_id}", CRITICAL, "report:template_manage"),
    # 规则和安全审计
    ...
    _cap("security.checklist", "/security", "查询安全审查清单", "GET", "/api/security/checklist"),
    _cap("security.dashboard", "/security", "查询安全态势汇总", "GET", "/api/security/dashboard-summary"),
    _cap("security.findings", "/security", "查询已落库安全发现", "GET", "/api/security/findings"),
    _cap("security.scan_file", "/security", "扫描单个代码文件", "POST", "/api/security/scan-file", WRITE),
    _cap("security.scan_task", "/security", "复审审查任务的安全发现", "POST", "/api/security/scan-task", WRITE),
    _cap("security.scan_project", "/security", "执行项目级白盒安全审计", "POST", "/api/security/scan-project", WRITE),
    _cap("security.scan_all_projects", "/security", "扫描当前用户可审计的全部项目", "POST", "/api/security/scan-all-projects", WRITE),
    # Agent 中心和用户拥有的 Agent/Skill 工坊资产
    _cap("agents.list", "/agents", "查询 Agent 画像", "GET", "/api/agents", permission="agent:view"),
    _cap("agents.type_mappings", "/agents", "查询审查类型与 Agent 映射", "GET", "/api/agents/type-mappings", permission="agent:view"),
    ...
    _cap("agents.runtime_summary", "/agents", "查询运行时 Agent 汇总", "GET", "/api/agents/runtime/summary", permission="agent:view"),
    _cap("agents.situation", "/agents", "查询 Agent 态势", "GET", "/api/agents/situation", permission="agent:view"),
    _cap("agents.skills", "/agents", "查询指定 Agent 的 Skill", "GET", "/api/agents/{agent_name}/skills", permission="agent:view"),
    _cap("agents.metagpt_info", "/agents", "查询 MetaGPT 编排层信息", "GET", "/api/agents/metagpt/info", permission="agent:view"),
    _cap("agents.metagpt_preview", "/agents", "预览 MetaGPT 编排环境", "GET", "/api/agents/metagpt/preview", permission="agent:view"),
    _cap("agent_catalog.list", "/agents", "查询已发布 Agent 目录", "GET", "/api/agent-catalog", permission="custom_agent:invoke"),
    _cap("agent_catalog.invoke", "/agents", "调用已发布的自定义 Agent", "POST", "/api/agent-catalog/{agent_code}/invoke", READ, "custom_agent:invoke"),
    _cap("agent_studio.agents.list", "/agent-studio", "查询我创建的 Agent", "GET", "/api/agent-studio/agents", permission="agent_asset:update_own"),
    _cap("agent_studio.agents.create", "/agent-studio", "创建自定义 Agent", "POST", "/api/agent-studio/agents", WRITE, "agent_asset:create"),
    _cap("agent_studio.agent_versions.list", "/agent-studio", "查询 Agent 版本", "GET", "/api/agent-studio/agents/{agent_id}/versions", permission="agent_asset:update_own"),
    _cap("agent_studio.agent_versions.get", "/agent-studio", "查询 Agent 版本详情", "GET", "/api/agent-studio/agent-versions/{version_id}", permission="agent_asset:update_own"),
    _cap("agent_studio.agent_versions.create", "/agent-studio", "创建 Agent 修订版", "POST", "/api/agent-studio/agents/{agent_id}/versions", WRITE, "agent_asset:update_own"),
    _cap("agent_studio.agent_versions.bind_skill", "/agent-studio", "绑定 Skill 到 Agent 版本", "POST", "/api/agent-studio/agent-versions/{version_id}/skills", WRITE, "agent_asset:update_own"),
    _cap("agent_studio.agent_versions.test", "/agent-studio", "测试 Agent 版本", "POST", "/api/agent-studio/agent-versions/{version_id}/test", WRITE, "agent_asset:test"),
    _cap("agent_studio.agent_versions.submit", "/agent-studio", "提交 Agent 版本发布审批", "POST", "/api/agent-studio/agent-versions/{version_id}/submit", WRITE, "agent_asset:submit"),
    _cap("agent_studio.agent_versions.withdraw", "/agent-studio", "撤回 Agent 版本发布申请", "POST", "/api/agent-studio/agent-versions/{version_id}/withdraw", WRITE, "agent_asset:submit"),
    _cap("agent_studio.skills.list", "/agent-studio", "查询我创建的 Skill", "GET", "/api/agent-studio/skills", permission="skill_asset:update_own"),
    _cap("agent_studio.skills.create", "/agent-studio", "创建自定义 Skill", "POST", "/api/agent-studio/skills", WRITE, "skill_asset:create"),
    _cap("agent_studio.skill_versions.list", "/agent-studio", "查询 Skill 版本", "GET", "/api/agent-studio/skills/{skill_id}/versions", permission="skill_asset:update_own"),
    _cap("agent_studio.skill_versions.get", "/agent-studio", "查询 Skill 版本详情", "GET", "/api/agent-studio/skill-versions/{version_id}", permission="skill_asset:update_own"),
    _cap("agent_studio.skill_versions.create", "/agent-studio", "创建 Skill 修订版", "POST", "/api/agent-studio/skills/{skill_id}/versions", WRITE, "skill_asset:update_own"),
    _cap("agent_studio.bindings.delete", "/agent-studio", "解绑 Agent 版本的 Skill", "DELETE", "/api/agent-studio/bindings/{binding_id}", CRITICAL, "agent_asset:update_own"),
    # 论坛、知识库和服务支持
    ...
    _cap("maintenance.close", "/support/maintenance", "关闭或撤销自己的维修工单", "POST", "/api/maintenance/{ticket_id}/close", WRITE),
