...
from app.utils.image_utils import sniff_image_mime, validate_image_bytes
...
BUILTIN_AVATAR_KEYS = frozenset(
    {"cat", "fox", "panda", "bunny", "shiba", "penguin", "owl", "dino", "chick", "jelly", "robot", "astro"}
)
_sniff_image_mime = sniff_image_mime
...
def set_avatar(payload: AvatarSetIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if value != "upload" and key not in BUILTIN_AVATAR_KEYS:
...
async def upload_avatar_image(file: UploadFile, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    try:
        mime = validate_image_bytes(raw, max_dimension=MAX_AVATAR_DIMENSION)
    except ValueError as exc:
        raise BadRequestError(str(exc), code=40001) from exc
...
def get_avatar_image(user_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """下发用户自定义头像(内联,带鉴权);内置/默认头像由前端本地渲染。"""
    from fastapi.responses import Response as FastResponse

    from app.services.rbac_service import is_admin_user

    if user_id != user.id and not is_admin_user(db, user.id):
        raise ForbiddenError("只能读取本人的头像", code=40300)
