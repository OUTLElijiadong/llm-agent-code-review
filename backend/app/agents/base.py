...
class BaseAgent:
    ...
    def call(self, user_message: str, ctx: Optional[AgentContext] = None,
             json_mode: bool = False) -> AgentResult:
        """调用 DeepSeek API
        ...
        self._emit(AgentEventType.THINKING, ctx,
                   ...
                   payload={"model": self._model, "json_mode": json_mode})
    ...
    def call_json(self, user_message: str, ctx: Optional[AgentContext] = None) -> AgentResult:
        """调用并解析为 JSON"""
        result = self.call(user_message, ctx, json_mode=True)
