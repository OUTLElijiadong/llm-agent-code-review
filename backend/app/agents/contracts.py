...
_CONTRACTS = (
    ...
    _contract(
        ...
        "管理 Agent",
        "protected_service",
        "作为管理员总调度入口，基于真实事实调用全部已启用 Agent，并维护确认和审计闭环。",
        ("规划管理员意图", "选择并委派专业 Agent", "维护聊天确认和执行回执"),
        ("调用全部已启用 Agent", "沿用确定性管理工具链", "汇总可追溯结论"),
        ("不得绕过高风险确认", "不得编造状态和数字", "不得读取用户私有内容"),
        (
            _skill(
                "manager.existing_admin_tools",
                "既有管理工具",
                "沿用 AdminCopilot 与 AdminAgentTools 的工具集合",
                "仅沿用既有实现",
            ),
        ),
