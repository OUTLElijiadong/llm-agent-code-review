    ...
    def generate(
        ...
        *,
        ...
        user_message = (
            ...
            "   python→blackbox.py(urllib)、node→blackbox.js;探测脚本用 http://127.0.0.1:{port}\n"
            "   请求(端口取环境变量 PRISM_PREVIEW_PORT),在应用稳定运行后执行,必须包含 DAST\n"
            "   注入渗透探测:SQL 注入(带 ' OR '1'='1 等 payload)、XSS(payload 回显检测)、\n"
            "   SSRF(仅限 127.0.0.1 回环探测)、越权(未授权访问受保护路径)、目录/错误页泄露\n"
            "   探测;每个探测记录状态码与响应片段,断言安全基线(如敏感错误不泄露),失败以\n"
            "   AssertionError/抛出异常表示。php 项目用 file_get_contents+stream context 或\n"
            "   curl 扩展发起回环请求。\n"
