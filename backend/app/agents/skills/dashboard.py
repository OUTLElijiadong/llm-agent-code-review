"""dashboard Agent 专属 Skill — 仪表盘 Agent 自进化与主动监测

DashboardSelfImprovementSkill: 从 ai_call_log 聚合 dashboard 调用统计,
产出指标阈值(评分等级/告警阈值)优化提案。

DashboardProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。
"""
...
class DashboardSelfImprovementSkill(SelfImprovementSkill):
    """dashboard Agent 自进化 Skill

    进化对象为指标阈值(评分等级/告警阈值)。aggregate_feedback 从 ai_call_log
    聚合调用统计,evolve_target 基于统计产出优化提案
    (失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "dashboard.self_improve"
    """

    name = "dashboard.self_improve"
    description = "从调用统计进化指标阈值(评分等级/告警阈值)"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "dashboard",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"仪表盘调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "dashboard_prompt_optimize",
                        "rule_name": "仪表盘提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 dashboard 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"仪表盘平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "dashboard_logic_simplify",
                        "rule_name": "仪表盘逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 dashboard 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class DashboardProactiveSkill(ProactiveSkill):
    """dashboard Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "dashboard.proactive"
    """

    name = "dashboard.proactive"
    description = "主动监测仪表盘调用趋势,触发自进化或提问建议"
