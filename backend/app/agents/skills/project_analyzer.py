"""project_analyzer Agent 专属 Skill — 项目分析 Agent 自进化与主动监测

ProjectAnalyzerSelfImprovementSkill: 从 ai_call_log 聚合 project_analyzer 调用统计,
产出项目结构识别模式优化提案。

ProjectAnalyzerProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。
"""
...
class ProjectAnalyzerSelfImprovementSkill(SelfImprovementSkill):
    """project_analyzer Agent 自进化 Skill

    进化对象为项目结构识别模式。aggregate_feedback 从 ai_call_log 聚合调用统计,
    evolve_target 基于统计产出优化提案(失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "project_analyzer.self_improve"
    """

    name = "project_analyzer.self_improve"
    description = "从调用统计进化项目结构识别模式"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "project_analyzer",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"项目分析调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "project_analyzer_prompt_optimize",
                        "rule_name": "项目分析提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 project_analyzer 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"项目分析平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "project_analyzer_logic_simplify",
                        "rule_name": "项目分析逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 project_analyzer 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class ProjectAnalyzerProactiveSkill(ProactiveSkill):
    """project_analyzer Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "project_analyzer.proactive"
    """

    name = "project_analyzer.proactive"
    description = "主动监测项目分析调用趋势,触发自进化或提问建议"
    ...
    def reflect_from_logs(
        ...
        try:
            ...
            if fail_rate > 0.3:
                reflections.append({
                    "finding": "project_analyzer 调用失败率偏高",
