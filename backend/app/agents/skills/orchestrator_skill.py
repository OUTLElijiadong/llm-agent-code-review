"""orchestrator Agent 专属 Skill — 主调度 Agent 自进化与主动监测

OrchestratorSelfImprovementSkill: 从 ai_call_log 聚合 orchestrator 调用统计,
产出 Agent 路由策略(意图→Agent 映射)优化提案。

OrchestratorProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。

注意:本文件名为 orchestrator_skill.py,但 agent_name 为 "orchestrator"。
"""
...
class OrchestratorSelfImprovementSkill(SelfImprovementSkill):
    """orchestrator Agent 自进化 Skill

    进化对象为 Agent 路由策略(意图→Agent 映射)。aggregate_feedback 从 ai_call_log
    聚合调用统计,evolve_target 基于统计产出优化提案
    (失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "orchestrator.self_improve"
    """

    name = "orchestrator.self_improve"
    description = "从调用统计进化 Agent 路由策略(意图→Agent 映射)"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "orchestrator",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"主调度调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "orchestrator_prompt_optimize",
                        "rule_name": "主调度提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 orchestrator 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"主调度平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "orchestrator_logic_simplify",
                        "rule_name": "主调度逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 orchestrator 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class OrchestratorProactiveSkill(ProactiveSkill):
    """orchestrator Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "orchestrator.proactive"
    """

    name = "orchestrator.proactive"
    description = "主动监测主调度调用趋势,触发自进化或提问建议"
