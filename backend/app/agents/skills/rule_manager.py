"""rule_manager Agent 专属 Skill — 规则管理 Agent 自进化与主动监测

RuleManagerSelfImprovementSkill: 从 ai_call_log 聚合 rule_manager 调用统计,
产出规则元数据(分类/严重度映射)优化提案。

RuleManagerProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。
"""
...
class RuleManagerSelfImprovementSkill(SelfImprovementSkill):
    """rule_manager Agent 自进化 Skill

    进化对象为规则元数据(分类/严重度映射)。aggregate_feedback 从 ai_call_log
    聚合调用统计,evolve_target 基于统计产出优化提案
    (失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "rule_manager.self_improve"
    """

    name = "rule_manager.self_improve"
    description = "从调用统计进化规则元数据(分类/严重度映射)"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "rule_manager",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"规则管理调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "rule_manager_prompt_optimize",
                        "rule_name": "规则管理提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 rule_manager 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"规则管理平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "rule_manager_logic_simplify",
                        "rule_name": "规则管理逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 rule_manager 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class RuleManagerProactiveSkill(ProactiveSkill):
    """rule_manager Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "rule_manager.proactive"
    """

    name = "rule_manager.proactive"
    description = "主动监测规则管理调用趋势,触发自进化或提问建议"
