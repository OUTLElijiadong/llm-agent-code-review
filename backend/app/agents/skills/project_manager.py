"""project_manager Agent 专属 Skill — 项目管理 Agent 自进化与主动监测

ProjectManagerSelfImprovementSkill: 从 ai_call_log 聚合 project_manager 调用统计,
产出项目配置模板优化提案。

ProjectManagerProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。
"""
...
class ProjectManagerSelfImprovementSkill(SelfImprovementSkill):
    """project_manager Agent 自进化 Skill

    进化对象为项目配置模板。aggregate_feedback 从 ai_call_log 聚合调用统计,
    evolve_target 基于统计产出优化提案(失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "project_manager.self_improve"
    """

    name = "project_manager.self_improve"
    description = "从调用统计进化项目配置模板"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "project_manager",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"项目管理调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "project_manager_prompt_optimize",
                        "rule_name": "项目管理提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 project_manager 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"项目管理平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "project_manager_logic_simplify",
                        "rule_name": "项目管理逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 project_manager 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class ProjectManagerProactiveSkill(ProactiveSkill):
    """project_manager Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "project_manager.proactive"
    """

    name = "project_manager.proactive"
    description = "主动监测项目管理调用趋势,触发自进化或提问建议"
    ...
    def reflect_from_logs(
        ...
        try:
            ...
            if fail_rate > 0.3:
                reflections.append({
                    "finding": "project_manager 调用失败率偏高",
