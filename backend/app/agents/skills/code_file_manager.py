"""code_file_manager Agent 专属 Skill — 文件管理 Agent 自进化与主动监测

CodeFileManagerSelfImprovementSkill: 从 ai_call_log 聚合 code_file_manager 调用统计,
产出文件分类规则优化提案。

CodeFileManagerProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。
"""
...
class CodeFileManagerSelfImprovementSkill(SelfImprovementSkill):
    """code_file_manager Agent 自进化 Skill

    进化对象为文件分类规则。aggregate_feedback 从 ai_call_log 聚合调用统计,
    evolve_target 基于统计产出优化提案(失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "code_file_manager.self_improve"
    """

    name = "code_file_manager.self_improve"
    description = "从调用统计进化文件分类规则"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "code_file_manager",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"文件管理调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "code_file_manager_prompt_optimize",
                        "rule_name": "文件管理提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 code_file_manager 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"文件管理平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "code_file_manager_logic_simplify",
                        "rule_name": "文件管理逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 code_file_manager 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class CodeFileManagerProactiveSkill(ProactiveSkill):
    """code_file_manager Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "code_file_manager.proactive"
    """

    name = "code_file_manager.proactive"
    description = "主动监测文件管理调用趋势,触发自进化或提问建议"
    ...
    def reflect_from_logs(
        ...
        try:
            ...
            if fail_rate > 0.3:
                reflections.append({
                    "finding": "code_file_manager 调用失败率偏高",
