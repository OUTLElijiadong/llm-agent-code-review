"""ai_prompt Agent 专属 Skill — 提示词生成 Agent 自进化与主动监测

AiPromptSelfImprovementSkill: 从 ai_call_log 聚合 ai_prompt 调用统计,
产出提示词模板(target_tool 模板)优化提案。

AiPromptProactiveSkill: 主动监测近 7 天调用失败率,触发进化或提问。
"""
...
class AiPromptSelfImprovementSkill(SelfImprovementSkill):
    """ai_prompt Agent 自进化 Skill

    进化对象为提示词模板(target_tool 模板)。aggregate_feedback 从 ai_call_log
    聚合调用统计,evolve_target 基于统计产出优化提案
    (失败率高→优化提示词,耗时高→简化逻辑)。

    Attributes:
        name: Skill 唯一标识 "ai_prompt.self_improve"
    """

    name = "ai_prompt.self_improve"
    description = "从调用统计进化提示词模板(target_tool 模板)"
    ...
    def aggregate_feedback(
        ...
        rows = (
            db.query(AiCallLog)
            .filter(
                AiCallLog.agent_label == "ai_prompt",
        ...
        for s in stats:
            if s.get("fail_rate", 0) >= 0.3:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"提示词生成调用失败率偏高({s['fail_rate']:.0%}),"
                        f"建议优化提示词"
                    ),
                    "payload": {
                        "rule_code": "ai_prompt_prompt_optimize",
                        "rule_name": "提示词生成提示词优化",
                        "rule_type": "prompt",
                        "rule_content": "优化 ai_prompt 的 system prompt,降低失败率",
                        "language": "*",
                        "severity": "中",
                    },
            ...
            if s.get("avg_duration_ms", 0) >= 10000:
                proposals.append({
                    ...
                    "target_rule_id": None,
                    "title": (
                        f"提示词生成平均耗时偏高({s['avg_duration_ms']}ms),"
                        f"建议简化逻辑"
                    ),
                    "payload": {
                        "rule_code": "ai_prompt_logic_simplify",
                        "rule_name": "提示词生成逻辑简化",
                        "rule_type": "performance",
                        "rule_content": "简化 ai_prompt 的处理逻辑,降低耗时",
                        "language": "*",
                        "severity": "低",
                    },
                    "evidence": {"source": "ai_call_log", **s},
...
class AiPromptProactiveSkill(ProactiveSkill):
    """ai_prompt Agent 主动行为 Skill

    主动监测近 7 天调用失败率,若失败率突增则触发进化或提问。

    Attributes:
        name: Skill 唯一标识 "ai_prompt.proactive"
    """

    name = "ai_prompt.proactive"
    description = "主动监测提示词生成调用趋势,触发自进化或提问建议"
