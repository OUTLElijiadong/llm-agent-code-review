...
def test_db_backup_disabled_skips(db: Any, monkeypatch: Any) -> None:
    """backup_schedule_enabled=False 时备份任务直接跳过，不调用运维动作。"""
    ...
    class FakeAgent:
        def execute_action(self, db_, actor, *, action, params=None, request_id="", session_db_id=None, source=""):
            called.append(action)
            return SimpleNamespace(success=True, data={"status": "success", "id": 1}, error=None)

    monkeypatch.setattr("app.agents.operations_agent.OperationsAgent", FakeAgent)
    job = SimpleNamespace(id=7, job_code="db_backup", job_type="db_backup", agent_code="operations")

    result = scheduler_service._execute_db_backup(db, job)

    assert result["skipped"] is True
    assert called == []


def test_db_backup_enabled_success(db: Any, monkeypatch: Any) -> None:
    """开启后备份任务调用 backup_database 且 source=scheduler，成功不告警。"""
    from app.core.config import settings
    ...
    class FakeAgent:
        def execute_action(self, db_, actor, *, action, params=None, request_id="", session_db_id=None, source=""):
            captured.update({"action": action, "actor": actor, "source": source})
            return SimpleNamespace(success=True, data={"status": "success", "id": 11}, error=None)

    monkeypatch.setattr("app.agents.operations_agent.OperationsAgent", FakeAgent)
    job = SimpleNamespace(id=8, job_code="db_backup", job_type="db_backup", agent_code="operations")

    result = scheduler_service._execute_db_backup(db, job)

    assert result["success"] is True
    assert captured["action"] == "backup_database"
    assert captured["actor"] is None
    assert captured["source"] == "scheduler"


def test_db_backup_failure_creates_critical_alert(db: Any, monkeypatch: Any) -> None:
    """备份失败时写入 critical 告警并推送 SSE 给超级管理员。"""
    from app.core.config import settings
    from app.models.agent_governance import AgentAlert

    monkeypatch.setattr(settings, "backup_schedule_enabled", True)
    emitted: list[tuple] = []
    monkeypatch.setattr("app.agents.event_bus.emit_event", lambda *a, **k: emitted.append((a, k)))
    ...
    class FakeAgent:
        def execute_action(self, db_, actor, *, action, params=None, request_id="", session_db_id=None, source=""):
            return SimpleNamespace(success=False, data={"status": "failed", "error": "OOM"}, error="备份失败: OOM")

    monkeypatch.setattr("app.agents.operations_agent.OperationsAgent", FakeAgent)
    job = SimpleNamespace(id=9, job_code="db_backup", job_type="db_backup", agent_code="operations")

    result = scheduler_service._execute_db_backup(db, job)

    assert result["success"] is False
    alert = db.query(AgentAlert).filter(AgentAlert.fingerprint == "backup:auto_failed").one()
    assert alert.severity == "critical"
    assert alert.category == "backup"
    assert alert.user_id == super_user.id
    assert emitted, "备份失败应推送 SSE 告警"
