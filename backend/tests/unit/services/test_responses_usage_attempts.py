...
async def test_native_non_200_usage_reaches_failed_round(monkeypatch):
    ...
    monkeypatch.setattr(
        service.httpx, "AsyncClient", lambda **kw: original(transport=httpx.MockTransport(handler), **kw)
    )
...
async def test_terminal_without_output_keeps_usage_and_delta_text():
    ...
    async def stream():
        ...
        yield {
            ...
            "response": {"id": "final", "status": "completed", "model": "actual-model", "usage": {"total_tokens": 7}},
        }
...
async def test_native_sink_cancel_after_terminal_read_keeps_usage(monkeypatch):
    ...
    def handler(_request):
        return httpx.Response(
            200, text=f"data: {json.dumps(terminal)}\n\n", headers={"content-type": "text/event-stream"}
        )
    ...
    transport = NativeResponsesTransport(
        ApiConfig(api_key="unit", base_url="https://unit.example/v1", model="unit"), cancel_sink
    )
    with pytest.raises(asyncio.CancelledError):
