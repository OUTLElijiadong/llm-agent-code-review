...
NAMES_ZH = ["失效的访问控制", "安全配置错误", "软件供应链失败", "加密失败",
            "注入", "不安全的设计", "身份验证失败", "软件或数据完整性失败",
            "安全日志与告警失败", "异常条件处理不当"]
...
def build(source: Path, verified_at: str) -> dict:
    ...
    for kind in ("Weakness", "Category", "View"):
        for node in root.findall(f".//c:{kind}", NS):
            ...
            entries[f"CWE-{node.attrib['ID']}"] = {
                "name": node.attrib["Name"], "kind": kind,
    ...
    for position, path in enumerate(paths):
        ...
        categories.append({
            "code": code, "name_en": name, "name_zh": NAMES_ZH[position],
            "owasp": f"{code}:2025-{name}", "cwe_refs": refs,
    ...
    metadata = {
        "owasp_version": "2025", "owasp_release": "Final",
        "owasp_source_url": "https://owasp.org/Top10/2025/",
        "cwe_version": root.attrib["Version"], "cwe_release_date": root.attrib["Date"],
        ...
        "mapping_note": "OWASP 各分类页面合计列出 249 个 CWE 标识；按 MITRE 4.20 类型核对为 246 个 Weakness 和 3 个 Category（16、320、1035）。官网介绍页另称 248，本库按可复核列表计数。",
...
def main() -> None:
    ...
    if advisory_file.exists():
        ...
        catalog["metadata"]["cve_scope"] = f"已核验 {len(records)} 条 PHP 生态 CVE 参考记录；不代表全量 CVE/NVD 数据库，也不代表项目已命中漏洞。"
        ...
        lines = ["# PHP 生态漏洞参考库", "", f"核验日期：{args.verified_at}。共 {len(records)} 条参考记录，来源为 CVE 官方记录和组件维护者公告。",
        ...
        for record in records:
            lines.extend([f"## 漏洞参考：{record['id']}",
                          f"- **来源标题**：{record['title']}",
                          f"- **组件与受影响范围**：{record['affected_summary']}",
                          f"- **判定依据（官方原文）**：{record['description']}",
                          f"- **官方来源**：{record['source_url']}",
                          f"- **来源类型**：{record['source_kind']}；记录更新时间：{record.get('updated_at') or '来源未提供'}。", ""])
        lines.extend(["## 历史资料纠正", "",
    ...
    frontend.write_text(json.dumps({key: catalog[key] for key in ("metadata", "categories")}, ensure_ascii=False, indent=2) + "\n")
