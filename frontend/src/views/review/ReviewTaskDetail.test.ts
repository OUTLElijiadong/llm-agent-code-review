...
describe('审查详情真实执行状态', () => {
  ...
  it('任务不可用显示审查语义、请求编号和返回入口，不读取问题列表', async () => {
    route.params.id = '162'
    ...
    expect(alert.text()).toContain('审查任务不存在或当前账号无权访问')
    expect(alert.text()).toContain('review-unavailable-162')
  ...
  it('失败后的重试保持原因和可返回入口，连点不重复且成功后清除旧编号', async () => {
    review.getReviewTaskDetail.mockRejectedValueOnce({ message: '读取暂不可用', request_id: 'failed-read-21' })
    await renderDetail()
    const pending = deferred<TaskDetailOut>()
    review.getReviewTaskDetail.mockReturnValueOnce(pending.promise)
    await button('重新加载').trigger('click')
    ...
    await flushPromises()
    expect(wrapper.find('.page-error').exists()).toBe(false)
    expect(wrapper.text()).not.toContain('failed-read-21')
  ...
  it.each([40303, 40400])('已加载任务失去访问权（%s）后清除旧内容，丢弃迟到源码并允许重新读取', async (code) => {
    review.getReviewTaskDetail.mockResolvedValue(taskResult({ files: [
      { file_id: 31, project_id: 7, file_name: 'private.ts', language: 'typescript', line_count: 2 },
    ] }))
    const pendingCode = deferred<unknown>()
    files.getDetail.mockReturnValueOnce(pendingCode.promise)
    await renderDetail()
    await wrapper.get('.file-row').trigger('click')
    review.getReviewTaskDetail.mockRejectedValueOnce({ code, message: '任务不可访问', request_id: 'access-revoked' })
    await vi.advanceTimersByTimeAsync(3000)
    await flushPromises()
    expect(wrapper.get('.page-error').text()).toContain('access-revoked')
    expect(wrapper.find('.workbench').exists()).toBe(false)
    expect(wrapper.text()).not.toContain('隔离测试审查')
    pendingCode.resolve({ id: 31, file_name: 'private.ts', language: 'typescript', is_binary: 0, content: 'late-secret-code' })
    await flushPromises()
    review.getReviewTaskDetail.mockResolvedValue(taskResult({ status: 'success', files: [] }))
    await button('重新加载').trigger('click')
    await flushPromises()
    expect(wrapper.text()).not.toContain('late-secret-code')
    expect(wrapper.text()).not.toContain('private.ts')
    expect(wrapper.find('.page-error').exists()).toBe(false)
  })

  it('离开失败页后迟到的重试不能重启轮询或读取问题', async () => {
    review.getReviewTaskDetail.mockRejectedValueOnce({ message: '读取失败' })
    await renderDetail()
    const pending = deferred<TaskDetailOut>()
    review.getReviewTaskDetail.mockReturnValueOnce(pending.promise)
    await button('重新加载').trigger('click')
...
describe('问题查看访问失效的独立恢复边界', () => {
  const privateIssue = { id: 901, file_id: 31, file_name: 'private.ts', title: 'cached-private-finding', description: 'cached-private-finding', severity: '高', issue_type: 'security', status: 'unfixed' }
  const state = () => (wrapper.vm.$ as unknown as { setupState: Record<string, any> }).setupState
  it.each([40101, 40303, 40400])('问题请求%s清除旧问题/抽屉/迟到代码，但保留已授权任务摘要', async code => {
    review.getReviewTaskDetail.mockResolvedValue(taskResult({ status: 'success' }))
    review.getTaskIssues.mockResolvedValueOnce({ items: [privateIssue], total: 1 })
    await renderDetail()
    const pending = deferred<unknown>()
    files.getDetail.mockReturnValueOnce(pending.promise)
    await wrapper.get('.issue-row').trigger('click')
    expect(state().drawerVisible).toBe(true)
    review.getTaskIssues.mockRejectedValueOnce({ code, message: '问题访问已失效', request_id: 'issue-revoke-901' })
    await state().retryIssues(); await flushPromises()
    expect(state().task.id).toBe(21)
    expect(state().issues).toEqual([])
    expect(state().issueTotal).toBe(0)
    ...
    expect(wrapper.get('.issues-error').text()).not.toContain('已有内容为上次成功')
    pending.resolve({ id: 31, file_name: 'private.ts', language: 'typescript', is_binary: 0, content: 'late-private-code' })
    await flushPromises()
    expect(state().codeContent).toBe('')
    expect(state().loadingCode).toBe(false)
    state().onIssueClick(privateIssue)
    await state().onIssueReviewed(privateIssue)
    expect(state().selectedIssue).toBeNull()
    expect(state().drawerVisible).toBe(false)
  })
  it('问题503保留标明过期的内容，重试成功后恢复', async () => {
    review.getReviewTaskDetail.mockResolvedValue(taskResult({ status: 'success' }))
    review.getTaskIssues.mockResolvedValueOnce({ items: [privateIssue], total: 1 })
    await renderDetail()
    review.getTaskIssues.mockRejectedValueOnce({ code: 50300, message: '暂时不可用' })
    await state().retryIssues(); await flushPromises()
    expect(wrapper.text()).toContain('cached-private-finding')
    expect(wrapper.get('.issues-error').text()).toContain('已有内容为上次成功')
    review.getTaskIssues.mockResolvedValueOnce({ items: [], total: 0 })
    await state().retryIssues(); await flushPromises()
    expect(wrapper.find('.issues-error').exists()).toBe(false)
    expect(state().issues).toEqual([])
  })
