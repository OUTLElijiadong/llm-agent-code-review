...
function statusOf(message: AgentMeshMessage): ResponseToolCallStatus {
  ...
  if (message.status === 'acknowledged') return 'acknowledged'
...
export function agentMeshToolCalls(
  ...
  return (messages ?? []).map((message) => {
    ...
    return {
      ...
      direction: receiving ? 'receive' : 'send',
      traceId: message.trace_id,
      protocolStatus: message.status,
      ...
      resultPreview: JSON.stringify({
        ...
        ...(message.last_error ? { error: message.last_error } : {}),
      }),
      status: statusOf(message),
      error: failed ? (message.last_error || `消息已${message.status}`) : undefined,
    }
  })
}
