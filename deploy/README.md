...
首次初始化新数据库时，`.env` 中的 `INITIAL_ADMIN_PASSWORD` 必须是 12-32 个字符、不超过 72 字节的非默认强口令。该值只用于把无法登录的种子占位值初始化为 bcrypt 哈希；已有环境的真实 `admin` 密码不会被覆盖。
...
`deploy.sh` 在备份和构建前检查仓库、备份目录及 Docker 数据目录所在文件系统。沿用 `ops-check.sh` 的 `OPS_DISK_CRITICAL_PERCENT`（默认 95%）作为阻断阈值，同时每处要求至少 `DEPLOY_MIN_FREE_GIB`（默认 12 GiB）可用；85% 的常规告警本身不阻断。目录或容量无法采集、阈值配置非法也会拒绝发布。提高绝对保留量可在发布命令的环境中设置 `DEPLOY_MIN_FREE_GIB`，不能设置为 0 关闭门禁；大容量数据库仍需结合备份恢复验证和实际镜像体积评估余量。
...
前端镜像构建后，先把镜像 assets 同步到共享卷，成功后才启动新前端。不得把同步放回 `compose up -d frontend` 之后：该命令不等待入口脚本，两个 `cp -a` 会并发写卷而报 `File exists`。入口脚本仍保留默认 14 天的旧哈希资源，当前资源更新时间，回滚继续使用旧镜像自身资源；同步失败不忽略，仍进入发布失败和应用回滚流程。
